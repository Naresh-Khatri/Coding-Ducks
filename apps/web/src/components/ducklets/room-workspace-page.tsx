...
/**
 * Which surface this room is being rendered on. Plain collaborative ducklets
 * live at `/ducklets/[id]`; machine-coding interview rooms live at
 * `/interview/[id]`. The two are kept on separate routes so a plain ducklet can
 * never surface the Problem/Tests panels (and vice versa); a room opened on the
 * wrong route is redirected to its canonical URL below.
 */
export type RoomMode = "ducklet" | "interview";
...
export function RoomWorkspacePage({
  ...
  // An interview room is the one the server attached a machine-coding context
  // to (kind === "machine-coding"). This is the single signal the route guard
  // and the Problem-panel wiring both key off.
  const isInterviewRoom = !!ducklet?.machineCodingContext;
  ...
  // Keep each room on its canonical route. Interview rooms belong at
  // `/interview/[id]`, plain ducklets at `/ducklets/[id]`. `replace` (not push)
  // so the wrong-route URL doesn't linger in history — old `/ducklets/<id>`
  // links to interview rooms still resolve, they just land on `/interview`.
  useEffect(() => {
    if (!ducklet) return;
    if (mode === "ducklet" && isInterviewRoom) {
      router.replace(`/interview/${roomId}`);
    } else if (mode === "interview" && !isInterviewRoom) {
      router.replace(`/ducklets/${roomId}`);
    }
  }, [ducklet, isInterviewRoom, mode, roomId, router]);
  ...
  // Room opened on the wrong route — the effect above is navigating away. Show a
  // placeholder rather than briefly flashing the wrong layout.
  if (
    (mode === "ducklet" && isInterviewRoom) ||
    (mode === "interview" && !isInterviewRoom)
  ) {
    return (
      <div className="flex h-[100dvh] items-center justify-center">
  ...
  const machineCodingContext: MachineCodingContext | null =
    ducklet.machineCodingContext
      ? {
    ...
    <div className="flex h-[100dvh] flex-col">
      <header className="bg-muted/20 flex items-center justify-between gap-2 border-b px-2 py-2 sm:px-4">
        ...
        <div className="flex shrink-0 items-center gap-1 sm:gap-2">
          ...
          <ShareModal roomId={roomId} isOwner={isOwner} isPublic={isPublic} />
